...
> **✅ Implemented (v1.0.9).** Shipped as `PowerMode.AUTO` (now the default). Thermal status is polled
...
> **✅ Implemented (v1.0.9).** Under `PowerMode.AUTO`, `AdaptivePolicy` runs an AIMD loop on the
...
> **✅ Implemented (v1.0.9).** The encoder's `KEY_REPEAT_PREVIOUS_FRAME_AFTER` floor was raised from
...
|---|---|---|---|---|---|
| 1 | Thermal-adaptive fps/bitrate | High | Small–Med | Low | ✅ Done (v1.0.9) — `PowerMode.AUTO` |
| 2 | Adaptive bitrate (link) | High | Medium | Medium | ✅ Done (v1.0.9) — needs bike-test tuning |
| 5 | Idle encoder throttle | Medium (battery) | Medium | Medium | ✅ Done (v1.0.9) — watch 9 s timeout on-bike |
...
Ideas 1, 2 and 5 shipped together (v1.0.9) — they share plumbing: live `MediaCodec.setParameters` on
